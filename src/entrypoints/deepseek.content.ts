import { DeepSeekExtractor } from "@/lib/extractors/deepseek";
...
export default defineContentScript({
	...
	main() {
		...
		onMessage("detect", () => ({ platform: "deepseek", supported: true }));
		...
		onMessage("extract", async ({ data }) => {
			...
			if (data.format === "json" || data.format === "both") {
				const jsonResult = serializeToJson(result, "deepseek", url);
				json = jsonResult.raw;
			}
			if (data.format === "markdown" || data.format === "both") {
				markdown = serializeToMarkdown(result, "deepseek", url);
			}
